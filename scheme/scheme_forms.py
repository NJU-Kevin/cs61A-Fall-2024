...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        validate_form(body, 1)  # Ensure body is a list with at least one
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True  # The empty and evaluates to True
    while expressions.rest is not nil:
        first_expr = expressions.first
        val = scheme_eval(first_expr, env)
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    if expressions is nil:
        return False  # The empty or evaluates to False
    while expressions.rest is not nil:
        first_expr = expressions.first
        val = scheme_eval(first_expr, env)
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest is nil:
                ...
                # Evaluate the body of the clause
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not isinstance(binding, Pair) or len(binding) != 2:
            raise SchemeError('bad binding in let form')
        name, value = binding.first, binding.rest.first
        names_copy = names
        while names_copy is not nil:
            if names_copy.first == name:
                raise SchemeError('duplicate binding in let form: {0}'.format(name))
            names_copy = names_copy.rest
        if not scheme_symbolp(name):
            raise SchemeError('non-symbol in let binding: {0}'.format(name))
...
def do_quasiquote_form(expressions, env, level=1):
