...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def about_select(paragragh):
        for word in split(lower(remove_punctuation(paragragh.strip()))):
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    if len(typed) == 0 and len(source) == 0: return 100.0
    elif ((len(typed) == 0 and len(source) > 0) or
          (len(typed) > 0 and len(source) == 0)): return 0.0
    else:
        all_count, acc_count = len(typed_words), 0
        for i in range(min(len(source_words), len(typed_words))):
            if typed_words[i] == source_words[i]: acc_count += 1
        return (acc_count / all_count) * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (typed, source) not in cache:
            cache[(typed, source)] = (diff_function(typed, source, limit),
                                      limit)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list: return typed_word
        ...
        if min(diff, key = abs) > limit: return typed_word
        else:
...
def furry_fixes(typed, source, limit):
    ...
    if limit < 0:
        ...
        if min(len(typed), len(source)) == 1:
            if typed[0] == source[0]:
                return abs(len(source) - len(typed))
            else:
                return abs(len(source) - len(typed)) + 1
        else:
            if typed[0] == source[0]:
...
def final_diff(typed, source, limit):
    ...
    """I simply copy the minimum_mewtations to the following."""
    if limit < 0: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        return limit + 1
        # END
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        """i is the index of player, tpp[i] is the timestamps of i"""
        period = []
        for j in range(1, len(tpp[i])):
            """j is the index of timestamp of player i"""
            period.append(tpp[i][j] - tpp[i][j - 1])
        times.append(period)
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        min_index = 0
        for j in player_indices:
            if get_time(times, j, i) < get_time(times, min_index, i):
                min_index = j
        fw[min_index].append(words[i])
        """words[i] is associated to fw[min_index]"""
    return fw
    # END PROBLEM 10
